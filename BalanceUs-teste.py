...
def iniciar():
   ...
   tab_caixa = Frame(resultados, bg="#d6eeb9")
   tab_embrapa = Frame(resultados, bg="#d6eeb9")
   tab_afrafep = Frame(resultados, bg="#d6eeb9")
   
   #Create Tabs
   tab_unimed.grid(row=1, column=0)
   tab_cassi.grid(row=1, column=1)
   tab_caixa.grid(row=1, column=2)
   tab_embrapa.grid(row=1, column=3)
   tab_afrafep.grid(row=1, column=4)
   
   #Insert Tab
   resultados.add(tab_unimed, text="Unimed")
   resultados.add(tab_cassi, text="Cassi")
   resultados.add(tab_caixa, text="Caixa")
   resultados.add(tab_embrapa, text="Embrapa")
   resultados.add(tab_afrafep, text="Afrafep")
   ...
   calc(tab_unimed, "Unimed")
   calc(tab_cassi, "Cassi")
   calc(tab_caixa, "Caixa")
   calc(tab_embrapa, "Embrapa")
   calc(tab_afrafep, "Afrafep")
...
def switch(convenio):
    opcoes = {
        "Unimed": unimed_entrada.get(),
        "Cassi": cassi_entrada.get(),
        "Caixa": caixa_entrada.get(),
        "Embrapa": embrapa_entrada.get(),
        "Afrafep": afrafep_entrada.get()
    }
    return opcoes.get(convenio)


def calc(tab, convenio):

    #Lê os Excel e cria Database
    #Switch para convênios
    print(switch(convenio))
    convenio_database = pd.read_excel(switch(convenio))
    ...
    pacientes_confirmados = open("PacientesConfirmados-%s.txt" % convenio, "w+")
    pacientes_confirmados.write("Lista de Pacientes Confirmados - %s\n" % convenio)
    pacientes_error = open("PacientesError-%s" % convenio,"w+")
    pacientes_error.write("Lista de Pacientes com Erros - %s\n" % convenio)

    #para cada procedimento em convenio
    for x in range(0, len(convenio_database)):
       
        #Pega os dados de cada procedimento
        data = convenio_database.iloc[x, 0]
        nome_paciente = convenio_database.iloc[x, 1]
        procedimento = convenio_database.iloc[x, 2]
        valor = convenio_database.iloc[x, 3]
    ...
    #Texto
    unimed_valeria = LabelFrame(tab, text="Dra.Valéria:",font=('Arial', 18), bg="#d6eeb9")
    unimed_geruza = LabelFrame(tab, text="Dra.Geruza:",font=('Arial', 18), bg="#d6eeb9")
    unimed_laurise = LabelFrame(tab, text="Dra.Laurise:",font=('Arial', 18), bg="#d6eeb9")
